...
所有 HJMedia 代码遵循一致的约定 — 理解这些是阅读/编写任何代码的前提。
...
**基类 HJObject**（`src/utils/HJObject.h`）是所有类的根，提供：
- `HJ_DECLARE_PUWTR(ClassName)` 宏 — 自动生成 `using Ptr = shared_ptr`、`using Utr = unique_ptr`、`using Wtr = weak_ptr`
- `getGlobalName(prefix)` / `getGlobalID()` — 全局唯一名称和 ID 生成
- `sharedFrom(this)` — 增强的 shared_from_this，支持动态类型转换
- `HJMakeGlobalName(prefix)` / `HJMakeGlobalID()` — 便捷宏
...
错误码定义在 `src/utils/HJError.h`：`HJ_OK`、`HJ_EOF`、`HJErrInvalidParams`、`HJErrNotAlready`、`HJErrNotSupport`、`HJ_WOULD_BLOCK`、`HJ_IDLE` 等。
...
**4. 任务调度模型**：节点继承 `HJTask`，通过 `HJScheduler::asyncAlwaysTask()` 投递到 `HJExecutor`（专用线程）执行。`m_isBusy` 标记防止重入。`asyncSelf(delayTimeMS)` 支持延迟调度（用于 A/V 同步等场景）。同步操作（`done()`/`seek()`）使用 `scheduler->sync()` 确保调用返回时状态已更新。
